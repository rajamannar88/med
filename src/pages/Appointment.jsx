...
const Appointment = () => {
  ...
  const getAvailableSlots = async() => {
    ...
    // Continue until we have 7 days of slots
    for(let i = 0; daysAdded < 7; i++){
      ...
      if(today.getDate() === currentDate.getDate()){
        // Check if clinic hours are over for today
        if(today.getTime() >= endTime.getTime()){
          continue; // Skip today, clinic is closed
        }
        // Set current time for today with buffer
        currentDate.setHours(currentDate.getHours() > 10 ? currentDate.getHours() + 1 : 10)
        currentDate.setMinutes(currentDate.getMinutes() > 30 ? 30 : 0)
      }
      else {
        // For future dates, start at 10 AM
